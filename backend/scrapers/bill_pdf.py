...
def fetch_pdf_text(url: str, bill_no: str | None = None, max_pages: int = 12) -> str:
    """Fetch a bill PDF and extract text. Caches to disk so repeat runs and the
    demo do not depend on the network."""
    if url.startswith("/"):
        url = BASE + url
...
def enrich(bill: dict) -> dict:
    """Given a bill dict from the listing (with pdf_url), attach parsed PDF facts."""
    if not bill.get("pdf_url"):
        return bill
    text = fetch_pdf_text(bill["pdf_url"], bill.get("bill_no"))
    if not text:
        return {**bill, "pdf_parsed": False}
    return {**bill, "pdf_parsed": True, **parse_bill(text)}
